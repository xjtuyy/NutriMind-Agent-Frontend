...
import { getTrainingStatsApi } from '@/api/dashboard'
import {
...
import {
  ...
  normalizeTrainingStats, normalizeTrainingTaskDetail, normalizeTrainingTaskList,
...
async function loadPreview() {
  ...
  tasks.value = result.items; total.value = result.total; stats.value = normalizeTrainingStats(preview.trainingPreviewStats); models.value = normalizeTrainingModels(preview.trainingPreviewModels); previewLoaded = true
}
...
async function loadTasks(background = false) {
  ...
  listError.value = ''
  try {
    const [taskResult, statResult] = await Promise.allSettled([
...
@media(prefers-reduced-motion:reduce){.skeleton,.spin{animation:none}.progress-cell i,.primary-action,.secondary-action{transition:none}}
@media(max-width:1180px){.stats-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.model-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.charts-grid{grid-template-columns:1fr}}
